...
with ctx:
    ...
    gateway.add_endpoint("/data/<name>", func=rpc.data.get_product_detail, auth=False, validate=True)
    gateway.add_endpoint("/data/<name>/result", func=rpc.data.get_product_detail_filelist, auth=False,
                         validate=True)
    ...
    gateway.add_endpoint("/updatebackend", func=rpc.jobs.updatebackend, auth=True, validate=True,
                         methods=["POST"])
    gateway.add_endpoint("/evaluation/<max_files>", func=rpc.jobs.evaluate_data_resulthash, auth=False, validate=False)
